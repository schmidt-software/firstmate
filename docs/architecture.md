...
A separate, shared-daemon-level bug can make the `ci` step itself report `running` forever even after GitHub's checks are already green ([issue #3](https://github.com/schmidt-software/firstmate/issues/3)); `bin/fm-no-mistakes-ci-recover.sh` diagnoses and, with `--force`, recovers from it.
