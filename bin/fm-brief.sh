...
#   direct-PR    implement -> push + open PR via the forge's CLI (no pipeline) -> configured merge authority
...
3. Use gh-axi for GitHub operations, tea for Gitea operations, and chrome-devtools-axi for browser operations.
...
When it is implemented and committed, push your branch and open a PR with your forge's CLI (Rule 3), then append \`done: PR {url}\` to the status file and stop.
